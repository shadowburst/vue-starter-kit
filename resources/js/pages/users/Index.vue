...
import type { UserIndexProps, UserIndexRequest, UserIndexResource, UserOneOrManyRequest } from '@/types';
...
const selectedRows = ref<UserIndexResource[]>([]);
const rowsActions: DataTableRowsAction<UserIndexResource>[] = [
    {
        label: trans('trash'),
        icon: ArchiveIcon,
        ...
        callback: (items) =>
            alert({
                ...
                callback: () =>
                    router.delete<UserOneOrManyRequest>(route('users.trash'), {
    ...
    },
    {
        ...
        callback: (items) =>
            alert({
                ...
                callback: () =>
                    router.delete<UserOneOrManyRequest>(route('users.delete'), {
                        data: { ids: items.map(({ id }) => id) },
                        only: ['users'],
                        onSuccess: () => {
                            selectedRows.value = [];
...
const rowActions: DataTableRowAction<UserIndexResource>[] = [
    {
        type: 'href',
        label: trans('edit'),
        icon: PencilIcon,
        hidden: (user) => !user.can_update,
        href: (member) => route('users.edit', { member }),
    },
    {
        type: 'href',
        label: trans('view'),
    ...
    },
    {
        ...
        callback: (member) =>
            alert({
                ...
                callback: () =>
                    router.delete<UserOneOrManyRequest>(route('users.trash', { member }), {
                        only: ['users'],
                    }),
            }),
    },
    {
        ...
        callback: (member) =>
            alert({
                ...
                callback: () =>
                    router.patch<UserOneOrManyRequest>(route('users.restore', { member }), undefined, {
                        only: ['users'],
                    }),
            }),
    },
    {
        ...
        callback: (member) =>
            alert({
                ...
                callback: () =>
                    router.delete<UserOneOrManyRequest>(route('users.delete', { member }), {
                        only: ['users'],
                    }),
            }),
    },
...
const filters = useFilters<UserIndexRequest>(
    route('users.index'),
    {
        q: props.request.q ?? '',
